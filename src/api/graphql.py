...
from src.models import LightStatus as ModelLightStatus
from src.models import LightBulb as ModelLightBulb
...
async def call_restate(service_name: str, handler_name: str, data: Optional[dict] = None, send: bool = True, time_window_seconds: Optional[int] = None) -> str:
    endpoint = f"{RESTATE_ENDPOINT}/{service_name}/{handler_name}{'/send' if send else ''}"
...
class Query:
    @strawberry.field
    async def lightbulb(self, key: str) -> Optional[LightBulb]:
        try:
            result = await get_restate_output(key, "lightbulb_manager", "toggle_lightbulb")
...
class Mutation:
    @strawberry.mutation
    async def toggle_lightbulb(self, id: str, status: LightStatus) -> Optional[ActionSuccess]:
        try:
            logger.info("Toggling lightbulb with id: %s", id)
            key = await call_restate("lightbulb_manager", "toggle_lightbulb", {"id": id, "status": status.value.upper()})
